public class JvmComprehension {
    ...
    public static void main(String[] args) { // JvmComprehension.java компилируется в байт-код - файл JvmComprehension.class.
        ...
        int i = 1;                      // 1 - в фрейм метода main() помещается ссылка и значение примитивной переменной типа int;
        Object o = new Object();        // 2 - в куче выделяется память для объекта 'о' типа Object и в фрэйм метода main()
        // помещается ссылка на этот объект в куче;
    ...
    private static void printAll(Object o, int i, Integer ii) { //в стэке создаётся новый фрейм; в фрейм метода printAll()
        // помещаются ссылки на объекты 'о' и 'ii' и ссылка и значение примитивной переменной типа int
        Integer uselessVar = 700;                   // 5 - в куче выделяется память для объекта 'uselessVar' типа Integer
        // и в фрэйм метода main() помещается ссылка на этот объект в куче;
        System.out.println(o.toString() + i + ii);  // 6 - в стэке создаётся новый фрейм, туда помещаются ссылка на
        // объект 'ii' и ссылка и значение примитивной переменной типа int, а также в стэке создаётся ещё один фрейм метода toString();
    }
